/************************************************************************
* Description: Finite Impulse Response (FIR) filter calculations	*
*									*
* Comments: A lot of vector multiplications and array handling. The     *
*           program contains 12 loops, not context sensitive. The 	*
*	    number of iterations of the loops in the program is not     *
*	    input dependent						*
************************************************************************/
...
void vec_mpy1( short y[], const short x[], short scaler )
{
  ...
  _Pragma("loopbound min 150 max 150")
  for (i = 0; i < 150; i++) {
    y[i] += ((scaler * x[i]) >> 15);
  }
}
...
long int codebook( long int mask, long int bitchanged, long int numbasis, 
    long int codeword, long int g, const short *d, short ddim, short theta )
/*
